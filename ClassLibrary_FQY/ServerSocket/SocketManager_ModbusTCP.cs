...
{
    public class SocketManager_ModbusTCP
    {
        ...
        public SocketManager_ModbusTCP(int numConnections, int receiveBufferSize)
        {
            m_clientCount = 0;
        ...
        private void ProcessReceive(SocketAsyncEventArgs e)
        {
            try
            {
                ...
                if (e.BytesTransferred > 0 && e.SocketError == SocketError.Success)
                {
                    ...
                    do
                    {
                        //判断Modbus TCP的MBAP报文包的长度
                        byte[] lenBytes = token.Buffer.GetRange(4, 2).ToArray();
                        Array.Reverse(lenBytes);
                        int packageLen = BitConverter.ToInt16(lenBytes, 0);
                        if (packageLen > token.Buffer.Count - 6)
                        { //长度不够时,退出循环,让程序继续接收
                            break;
                        }
                        //包够长时,则提取出来,交给后面的程序去处理
                        byte[] rev = token.Buffer.GetRange(0, packageLen + 6).ToArray();
                        //从数据池中移除这组数据
                        lock (token.Buffer)
                        {
                            token.Buffer.RemoveRange(0, packageLen + 6);
                        }
